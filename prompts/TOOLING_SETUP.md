...
2. Diff/merge the global router and conditional references using [GLOBAL_REFERENCE_MERGE.md](../templates/setup/GLOBAL_REFERENCE_MERGE.md); preserve personal rules and copy TOOLS only within this explicitly authorized environment scope. Reconcile the global `/apm-setup` bootstrap from canonical `prompts/APM_SETUP.md` at the same time. It is the only agent-reference package command intended globally; do not globalize `/agent-sync` or `/test-setup`, and do not activate global Skills.
...
6. Restart the affected OpenCode process after preserving active work; run [direct and delegated capability checks](../evaluation/CAPABILITY_SMOKE.md) on the target host. Do not terminate unrelated sessions. If the global APM bootstrap changed, verify its bytes and actual OpenCode command discovery when the installed host exposes a safe discovery check; otherwise report host discovery `NOT RUN` rather than inferring it from file presence.
