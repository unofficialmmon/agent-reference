...
2. Execute [OPENCODE_PLUGIN_SETUP.md](OPENCODE_PLUGIN_SETUP.md) for approved plugins, MCPs, ast-grep, and OpenCode/OMO configuration. The existing filename remains the supported entry point.
3. Execute [DEVELOPER_CLI_SETUP.md](DEVELOPER_CLI_SETUP.md) for selected workstation utilities. Do not install a second copy under another manager.
4. Execute [PROJECT_TOOLING_SETUP.md](PROJECT_TOOLING_SETUP.md) only when a specific consumer repository is in scope. Repository bootstrap/refresh alone is not permission for global installs or security/Bot onboarding.
5. Restart the affected OpenCode process after preserving active work; run direct and delegated smoke checks on the target host. Do not terminate unrelated sessions.
6. Re-inspect installation and effective configuration. Report NOOP on an equivalent second run; do not rewrite files merely to stamp dates.
...
Produce one combined review report, not one repeated checklist per sub-prompt. Use Result / Changed / Validation / Needs attention. Separate installation, configuration, authentication, routing, direct execution and host behavior. Report exact versions and paths without secrets. A installed binary with untested behavior is not a host PASS. Required failed or blocked checks prevent a fully verified result. End after the report without opening a cleanup menu.
