...
- When a material current external fact can be retrieved safely from an available authoritative tool or MCP, retrieve it rather than guessing from model knowledge, memory, or an older conversation. Do not call external tools for facts already established by current repository evidence; load `TOOLS.md` when capability choice or trust boundaries matter.
